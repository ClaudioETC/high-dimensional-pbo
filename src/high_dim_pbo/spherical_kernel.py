...
#we sum over the last dimension (dim=-1); our only interest
...
#helper function for low-rank kernel matrix scenario 
...
class SphericalLinearKernel(gpytorch.kernels.RBFKernel):
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(bounds[0], float):
            #our bounds are [0,1] from the D-dimensional Hypercube
    ...
    def forward(self, x1: torch.Tensor, x2: torch.Tensor, diag: bool = False, **params):  # noqa: D102
        ...
        # Make sure that we're within bounds of the d-dim [0,1] hypercube
        ...
        # Get constants
        #don't really need lengthscale in our initial problem, yet we need it for ths RBF implementation
        #lengthscale is a learnable parameter in gpytorch kernels.
        lengthscale: Float[Tensor, "... 1 D"] = self.lengthscale
        #max_sq_norm calculates the high dimensional numerical problem. 
        # we scale each dimension by its lengthscale; in this case we scale 
        #the boundaries of our search space  
